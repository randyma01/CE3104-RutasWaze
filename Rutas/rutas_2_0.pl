...
/*** 1. Declaraciones	de validaciones necesarias.***/
:-dynamic (rpath/2).  % hacer modificaciones a la base de datos
 ...
 * de datos. En caso de que la
 * rutas sea dirigida elimina
 * la vía correspondiente. En
 * caso que sean no dirigidas,
 * elimina la ruta o la vía
 * en ambos sentidos.
 */
bloquearRuta(X,Y,Z):-
	retract(ruta(X,Y,Z)),
 ...
 * valorDistancia(+A,+B).
 *
 * Si existe una vía o camino
 * o ruta en el grafo en ambos
 * sentido, entonces hay una peso
 * correspondiente. Se retorna e
 * se peso entre los nodos.
 *
 */
valorPeso(X,Y):-
	ruta(Y,X,Z),
 ...
 * esta identificada con la
 * distancia total a recorrer
 * y las distancias entre los
 * puntos.
 */
direccionesDistancia(Path) :-
  [Head,Second|Tail] = Path,
  write(Second),
  write(" alrededor de unos: "),
  valorPeso(Head,Second),
  write("km"),
  write(" ,luego continue hasta "),
  write(Tail),
  direccionesDistancia(Tail).


/*
 * rutaDireccionada(+Inicio,+Fin)
 *
 * Encuenta la ruta con la menor
 * distancia entre el punto de
 * inicio y el final. Devuelve
 * la ruta a tomar con las dire-
 * cciones y distancias a tomar
 * para llegar aldestino.
 */
rutaDireccionada(Inicio, Fin) :-
	recorrer(Inicio),
	rpath([Fin|RRuta], Dist)->
	  ...
	  writef('La distancia total es de: %d km. Por favor continue hasta', [Distancia]),
	  direccionesDistancia(Ruta),
	  write('Ha llegado a su destino.'),
	write('Fin.').

/*
 * rutaNoDireccionada(+Inicio,+Fin)
 *
 * Encuentra la ruta con la menor
 * distancia entre el punto de
 * inicio y el final. Devuelve los
 * puntos a recorrer con la distancia
 * total.
 */
rutaNoDireccionada(Inicio, Fin) :-
	recorrer(Inicio),
	rpath([Fin|RRuta], Dist)->
	  reverse([Fin|RRuta], Ruta),
	  Distancia is round(Dist),
	  writef('La ruta más corta a recorrer es: %w, con una distancia de: %w = %wkm\n',
	       [Ruta, Dist, Distancia]);
	writef('Error, no existe una ruta de %w a %w\n', [Inicio, Fin]).

/*
 * demorando(+Inicio,+Fin,+Tiempo)
 *
 * Verifica que exista un arco,
 * arista o camino entre dos puntos
 * del grafo.
 ...
 * demorando(+Inicio,+Fin,+Tiempo)
 *
 * Verifica el arco, camino o
 * vía del grafo tenga la dirección
 * opuesta.
 */
 ...
 * puntos.
 */
direccionesTiempo(Path) :-
  [Head,Second|Tail] = Path,
  write(Second),
  write(" alrededor de unos: "),
  valorPeso(Head,Second),
  write("min"),
  write(" ,luego siga hasta llegar a "),
  write(Tail),
  direccionesTiempo(Tail).


/*
 * tiempoDireccionado(+Inicio,+Fin)
 *
 * Encuentra todas los tiempos
 * si encuentra el punto de fin
 * elabora el camino con el menor
 * tiempo y reporta los nodos
 * a recorrer con su respectivo
 * peso. Además da la dirección
 * a tomar con los respectivos
 * tiempos a demorar.
 *
 */
tiempoDireccionado(Inicio, Fin) :-
	...
	rpath([Fin|RRuta], Tiem)->
	  ...
	  writef('La ruta con el menor tiene una duración de %d min. Por favor tome la vía hacia', [Tiempo]),
	  direccionesTiempo(Ruta),
	  write('Ha llegado a su destino');
	 writef('Fin').


/*
 * tiempoNoDireccionado(+Inicio,+Fin)
 *
 * Encuentra todas los tiempos
 * si encuentra el punto de fin
 * elabora el camino con el menor
...
tiempoNoDireccionado(Inicio, Fin) :-
	...
	rpath([Fin|RRuta], Tiem)->
	  reverse([Fin|RRuta], Ruta),
	  Tiempo is round(Tiem),
	  writef('La ruta con el tiempo es: %w,	y tiene una demora de de: %w = %w\n min',[Ruta, Tiem, Tiempo]);
	writef('Error, no existe una ruta de %w a %w\n', [Inicio, Fin]).
